...
- `OTEL_EXPORTER_OTLP_ENDPOINT` / `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`: Standard OTel generic/signal-specific OTLP/HTTP configuration
...
8. **Tool-call failures are warnings with argument shapes only.** Do not log MCP
   ...
   Bind `tool_argument_shape=_mapping_shape(arguments)` so operators can diagnose
   malformed calls without retaining queries, identifiers, messages, or other client data.
   Authorization headers remain masked via `log_api_request()`.
...
- If either standard OTLP endpoint variable is set, traces export via OTLP/HTTP; otherwise a provider without an exporter is used (trace IDs still appear in logs).
- The exporter reads endpoint variables itself so generic endpoints receive the standard `/v1/traces` suffix.
- Starlette and HTTPX auto-instrumentation connect inbound MCP requests to outbound backend requests; exclude health endpoints in deployment configuration.
- All exported spans pass through the privacy sanitizer: remove headers, URL queries, user/session/resource identifiers, exception messages, and stack traces while preserving trace topology and `exception.type`.
- Disable FastMCP's built-in production logger (`FASTMCP_LOG_ENABLED=false`); its validation and exception records can retain rejected input. The CodeAlive middleware remains the safe tool lifecycle source.
...
- In `src/middleware/observability_middleware.py`, extract tool arguments from
  ...
  tool_argument_shape=_mapping_shape(tool_arguments)): ...`.
...
- In each tool body, log in-band validation failures before raising `ToolError`.
  Include only parameter shapes, never raw parameter values.

Concrete example: `src/tools/artifact_relationships.py::get_artifact_relationships`
must log these branches as Warning with argument shapes:

```python
tool_argument_shape = _mapping_shape({
...
4. Errors are logged as Warning with `tool_argument_shape` before they go through `handle_api_error(ctx, e, "description", method=_TOOL_NAME)` — this preserves diagnostics without storing client data
